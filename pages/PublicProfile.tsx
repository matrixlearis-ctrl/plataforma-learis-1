              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {pro.portfolioUrls.map((url, i) => (
                  <div key={i} className="aspect-square bg-gray-100 rounded-[2rem] overflow-hidden group border-4 border-white shadow-md">
                    <img src={url} alt={`Projeto ${i + 1}`} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-center py-12 bg-gray-50 rounded-[2rem] border-2 border-dashed border-gray-100 text-gray-400 font-bold">
                Nenhum projeto postado no portfólio ainda.
                <p className="text-[10px] uppercase tracking-widest mt-2">Fotos de portfólio expiram em 120 dias</p>
              </div>
            )}
          </div>
        </div>
